...
int main (
  ...
  for (size_t i = 0; i < n0; i++) {
    x0s[i] = 0.5 - 0.5 * cos(pi * i / (n0 - 1));
  }
  for (size_t i = 0; i < n1; i++) {
    x1s[i] = 0.5 - 0.5 * cos(pi * i / (n1 - 1));
  }
  if (0 != interp_cubic_initialise(n0, n1, x0s, x1s, &plan)) {
    goto abort;
  }
  ...
  if (0 != interp_cubic_execute(plan, y0s, y1s)) {
    goto abort;
  }
  if (0 != interp_cubic_finalise(&plan)) {
    goto abort;
  }
