...
static int test0 (
  ...
  // make a plan
  dst_plan_t * plan = NULL;
  if (0 != dst_init_plan(nitems, &plan)) {
    return 1;
  }
  // self check, forward followed by backward
  dst_exec_f(plan, buffers[0]);
  dst_exec_b(plan, buffers[0]);
  test_compare("dst2 followed by dst3", nitems, buffers[0], buffers[1]);
  // clean-up plan
  dst_destroy_plan(&plan);
  // clean-up buffers
  free(buffers[0]);
  free(buffers[1]);
  return 0;
...
static int test1 (
  ...
  // compare with naive dst2
  test_exec_f(nitems, buffers[0], buffers[2]);
  dst_exec_f(plan, buffers[1]);
  test_compare("dst2 compared with naive impl.", nitems, buffers[1], buffers[2]);
  // clean-up plan
  dst_destroy_plan(&plan);
  // clean-up buffers
  free(buffers[0]);
...
static int test2 (
  ...
  // compare with naive dst3
  test_exec_b(nitems, buffers[0], buffers[2]);
  dst_exec_b(plan, buffers[1]);
  test_compare("dst3 compared with naive impl.", nitems, buffers[1], buffers[2]);
  // clean-up plan
  dst_destroy_plan(&plan);
  // clean-up buffers
  free(buffers[0]);
