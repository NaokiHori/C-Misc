...
int rdft_exec_f (
  ...
  dft(nitems / 2, - 1., 1, table_cos, table_sin, (double complex *)xs, zs);
  ...
  for (size_t i = 0; i < nitems / 2 + 1; i++) {
    const double complex e = + 0.5 * zs[i] + 0.5 * conj(zs[nitems / 2 - i]);
    const double complex o = - 0.5 * zs[i] + 0.5 * conj(zs[nitems / 2 - i]);
...
int rdft_exec_b (
  ...
  double complex * const zs = plan->buf;
  for (size_t i = 0; i < nitems / 2; i++) {
    const double real0 =               xs[             i];
    const double imag0 = 0 == i ? 0. : xs[nitems     - i];
    const double real1 =               xs[nitems / 2 - i];
    const double imag1 = 0 == i ? 0. : xs[nitems / 2 + i];
  ...
  dft(nitems / 2, + 1., 1, table_cos, table_sin, zs, (double complex *)xs);
...
int rdft_init_plan (
  ...
  *buf = memory_alloc((nitems / 2 + 1) * sizeof(double complex));
