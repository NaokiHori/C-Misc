...
int rdft_exec_b (
  ...
  double complex * const zs = plan->buf;
  for (size_t i = 0; i < nitems / 2; i++) {
    const double complex e = + 0.5 * xs[i] + 0.5 * conj(xs[nitems / 2 - i]);
    const double complex o = + 0.5 * xs[i] - 0.5 * conj(xs[nitems / 2 - i]);
    const double c = table_cos[i];
    const double s = table_sin[i];
    const double complex twiddle = c + I * s;
    zs[i] = e + o * I * twiddle;
  }
  dft(nitems / 2, + 1., 1, table_cos, table_sin, zs, (double complex *)ys);
