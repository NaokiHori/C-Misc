...
static int check (
    const marching_squares_contour_t * contour
) {
  ...
  while (contour) {
    for (const marching_squares_point_t * point = contour->point; point; point = point->next) {
      fprintf(fp, "% .15e % .15e\n", point->x, point->y);
    }
    fprintf(fp, "\n");
...
int main (
  ...
  marching_squares_contour_t * contour = NULL;
  if (0 != construct_dataset(sizes[0], sizes[1], grids[0], grids[1], field)) {
    puts("failed to prepare dataset");
    retval = 1;
    goto abort;
  }
  // perform marching-squares algorithm to extract contours
  if (0 != marching_squares_exec(threshold, sizes, grids, field, &contour)) {
...
abort:
  // clean-up
  marching_squares_cleanup(&contour);
  free(grids[0]);
  free(grids[1]);
  free(field);
  return retval;
