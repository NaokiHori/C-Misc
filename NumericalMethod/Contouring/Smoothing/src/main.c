...
int main(
  ...
  for (size_t n = 0; n < npoints - 3; n++) {
    const double * const p0 = points[n + 0];
    const double * const p1 = points[n + 1];
    const double * const p2 = points[n + 2];
    const double * const p3 = points[n + 3];
    double xcoefs[4] = {0.};
    double ycoefs[4] = {0.};
    ...
    const size_t nsub = 128;
    for (size_t m = 0; m < nsub; m++) {
      const double t = 2. * m / (nsub - 1) - 1.;
      const double x = xcoefs[0] + t * (xcoefs[1] + t * (xcoefs[2] + xcoefs[3] * t));
      const double y = ycoefs[0] + t * (ycoefs[1] + t * (ycoefs[2] + ycoefs[3] * t));
      fprintf(fp, "% .7e % .7e\n", x, y);
    }
  }
  fclose(fp);
