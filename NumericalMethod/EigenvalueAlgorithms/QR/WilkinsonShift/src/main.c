...
static int compute_shift(
  ...
  const double diagonals[2] = {
    matrix[(nitems - 2) * nitems + (nitems - 2)],
    matrix[(nitems - 1) * nitems + (nitems - 1)],
  };
  const double sub_diagonal = matrix[(nitems - 1) * nitems + (nitems - 2)];
...
#define NITEMS 8
  ...
  for (size_t iter = 0; iter < (1 << 7); iter++) {
    ...
    // compute shift
    double shift = 0.;
    compute_shift(NITEMS, input, &shift);
    // apply shift
    for (size_t i = 0; i < NITEMS; i++) {
